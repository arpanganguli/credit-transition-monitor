...
@pytest.mark.parametrize(
    "pd_value, lgd, ead, expected_loss",
    [
...
def test_expected_loss(pd_value: float, lgd: float, ead: float, expected_loss: float) -> None:
    ...
    assert result["expected_loss"].iloc[0] == pytest.approx(
        expected_loss,
        rel=1e-9,
    )